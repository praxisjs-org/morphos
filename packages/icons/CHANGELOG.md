...
- 52f1c8a: Redesigned around a single `Icon` component that resolves icons by name, plus a provider system
...
svg }` map, a glob path string (with `@morphos/icons/vite`'s `iconsPlugin()` wired into
  ...
  - `IconInstance` — a PraxisJS `Composable` wrapping `getIconProvider`/`setIconProvider`, for
    ...
    class IconProviderSwitcher extends StatefulComponent {
      ...
      render() {
        return (
          <button onClick={() => this.iconInstance.setProvider("brand")}>
            Use brand icons
          </button>
        );
      }
...
- 5a57249: Initial release. New package for bringing your own icon set: `Icon` (generic SVG primitive for
  raw markup), `LucideIcon` (renders `lucide` package icon data), and `PhosphorIcon` (renders raw
  SVG assets from `@phosphor-icons/core`). Neither `lucide` nor `@phosphor-icons/core` is a
  dependency — install whichever icon set you use in your own app.
