...
| Method | Description |
|---|---|
| `isPressed(value: string)` | Returns whether `value` is currently selected (in the array for `"multiple"`, or the equal value for `"single"`). |
| `toggle(value: string)` | Selects/deselects `value` according to `type`. Emits `onValueChange` with the new value. |
