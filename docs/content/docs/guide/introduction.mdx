...
| `@morphos/icons` | `Icon` (render by name) + `IconProvider` (mandatory in every app — configures the provider; there's no default). `lucide` is a real peer dependency wrapped by the built-in `LucideSource` — no per-icon data wiring needed. Register your own icon set with `RegisterIconProvider`. |
