...
`imageLoaded` and `imageError` are also available as readonly getters (`boolean`) derived from the
current status, if you need to branch on load state outside of the built-in `AvatarImage` /
`AvatarFallback` visibility logic.
